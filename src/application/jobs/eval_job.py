...
_DATASET_PATH = (
    Path(__file__).parent.parent.parent / "__tests__" / "evaluation" / "golden_dataset.json"
)
...
async def run_eval_suite(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    ctx_rel_m = ContextualRelevancyMetric(
        threshold=_THRESHOLDS["contextual_relevancy"], model=judge
    )
    ctx_prec_m = ContextualPrecisionMetric(
        threshold=_THRESHOLDS["contextual_precision"], model=judge
    )
    ...
    summary["timestamp"] = datetime.now(tz=UTC).isoformat()
