...
_PROFILES: dict[str, UserProfile] = {
    "cliente1988": UserProfile(
        ...
        status="active",
        joined_at=datetime(2020, 3, 15, tzinfo=UTC),
    ),
    "cliente2024": UserProfile(
        plan="Get Clássica",
        machine_model="Clássica Plus",
        status="active",
        joined_at=datetime(2024, 1, 5, tzinfo=UTC),
    ),
    "cliente_suspenso": UserProfile(
        plan="Get Smart",
        machine_model="Smart 2",
        status="suspended",
        joined_at=datetime(2019, 6, 1, tzinfo=UTC),
    ),
    "cliente_novo": UserProfile(
        plan="Get Clássica",
        machine_model="Clássica",
        status="active",
        joined_at=datetime(2026, 7, 20, tzinfo=UTC),
    ),
    "empresa_xpto": UserProfile(
        plan="Get Smart Pro",
        machine_model="Smart Pro",
        status="active",
        joined_at=datetime(2018, 11, 30, tzinfo=UTC),
    ),
}
